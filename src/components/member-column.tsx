...
export function MemberColumn({ memberId, index }: MemberColumnProps) {
  ...
  console.log(
    ...
    member.id,
    member.name,
    (member.qrData || member.qrUrl!)?.slice(0, 80),
  );
  return (
      ...
      <div className={cn("rounded-t-xl px-3 py-2.5", colorClass)}>
        ...
        <div className="mt-2 flex items-center justify-center">
          <div
            ...
            onClick={() =>
              (member.qrData || member.qrUrl) && setQrViewOpen(true)
            }
          >
            {member.qrData || member.qrUrl ? (
              <img
                src={member.qrData || member.qrUrl!}
                alt={`QR ${member.name}`}
          ...
          <div className="flex flex-col items-center gap-4 py-4">
            ...
            {(member.qrData || member.qrUrl) && (
              <img
                src={member.qrData || member.qrUrl!}
                alt={`QR ${member.name}`}
                className="w-full max-w-[500px] rounded-lg object-contain bg-white"
              />
            )}
          </div>
        </DialogContent>
      ...
      <Dialog open={removeOpen} onOpenChange={setRemoveOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Sẽ xoá vĩnh viễn tất cả chi tiêu của người này. Không thể hoàn
              tác.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={clearOpen} onOpenChange={setClearOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Chỉ xoá các khoản chi, không xoá người này. Không thể hoàn tác.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
